...
let prevPrice = parseFloat(localStorage.getItem(`${$(this).attr("id")}TotalPrice`));
...
if (prevPrice == 0) {$(this).data("totalprice", $(this).data("price"));}
...
else {$(this).data("totalprice", prevPrice + $(this).data("price"))}
...
//on store dans le local storage toutes les donnees recolter (nom du produit ajouter, category, prix, total  du prix du produit, nombre de produit chaque donner est accessible par la clé associer (exemple `${$(this).attr("id")}TotalPrice`) est la cle pour avoir le prix total)
  ...
  localStorage.setItem(`${$(this).attr("id")}TotalPrice`, $(this).data("totalprice"));
...
//function qui affiche les elements mis dans le panier (leurs nom, quantité et le prix total)
